...
export const formatter = (indate, mode = true) => {
    const begin = {
        ...
        date = begin.isDate && begin.noNull ? indate : new Date(Date.now()),
        padtwo = num => num.toString().padStart(2, '0')
    try {
        if (mode) {
            const fullDate = date.toLocaleDateString(),
                dateReve = fullDate.split('/').reverse(),
                datePadd = dateReve.map(padtwo).join('-'),
                fullTime = date.toTimeString().split(' ')
            return `${datePadd} ${fullTime[0]}`
        } else {
            const allDates = [date.getFullYear(), date.getMonth(), date.getDate()],
                [year, month, day] = allDates.map(d => Number(padtwo(d)))
